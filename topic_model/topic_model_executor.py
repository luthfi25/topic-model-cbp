...
def execute(dataset_file, dataset_folder, background_file, ntopics):
    ...
    if os.path.isfile("dataset/"+dataset_folder+"/gt.dat") == False:
        #Generate Ground Truth Points
        os.system("./topic_model/src_lda -g -out={} -ks={} -P=7".format("dataset/"+dataset_folder, background_file))

    #Execute BK-LFLDA (or PKLFLDA, both can be used interchangeably)
    os.system("java -jar topic_model/PK-STTM.jar -model PKLFLDA -ntopics {} -alpha 0.15 -beta 0.01 -niters 2000 -twords 10 -name {} -mu 0.7 -sigma 0.3 -A 25 -ks {} -corpus {} -gt {} -lambda 0.4".format(ntopics, dataset_folder, background_file, dataset_file, "dataset/"+dataset_folder+"/gt.dat"))
